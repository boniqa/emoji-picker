angular.module('emoji-support').factory('emojiService', ["$window", function ($window){
    ...
    // get rid of all the skin tones
    _emojiList.forEach(function (emoji) {
      if(emoji.name.indexOf("skin tone")===-1){
        _strippedTones.push(emoji);
      }
    });
    // filter out all the duff windows emojis
    if ($window.navigator.platform === "Win32") {
      console.log("I'm on windows 🤷‍♂️");
      _strippedTones.forEach(function (emoji) {
        if (emoji.category === "flags" || unsupportedOnWin32.includes(emoji.output)) {
      ...
      emojiList = _strippedTones;
    }


    // TODO: remove all te flags and the category if on windows 
